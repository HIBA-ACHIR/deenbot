...
class LargeRequestMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        request._body_size_limit = (
            1024 * 1024 * 1024 * 2
        )  # 2GB (increased for longer videos)
...
async def fatwaask_endpoint(
    ...
    try:
        ...
        return {
            "answer": "عذراً، حدث خطأ أثناء معالجة سؤالك. يرجى المحاولة مرة أخرى بعد قليل."
        }
...
async def root():
    return {
        "message": "Welcome to DeenBot API",
        "status": "running",
        "version": "1.0.0",
    }
