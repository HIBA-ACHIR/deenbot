...
from dependencies.audio_processor import AudioProcessorError, AudioSplittingError, DiskSpaceError
...
def transcribe_uploaded_audio(file_path: str, fast_mode: bool = True) -> str:
    ...
    try:
        ...
        from dependencies.audio_processor import AudioProcessor
        ...
        if groq_api_key:
            ...
            try:
                ...
                prompt = ChatPromptTemplate.from_messages([
                    ("system", """أنت مساعد ذكي متخصص في استخراج المواضيع الرئيسية من النصوص الإسلامية. 
                    مهمتك هي قراءة النص المدخل وتحديد الموضوع الرئيسي له بدقة.
                    قم بإنشاء عنوان وصفي موجز يلخص محتوى النص في جملة واحدة.
                    لا تستخدم كلمة 'موضوع' أو 'عنوان' في إجابتك، بل اكتب العنوان مباشرة.
                    يجب أن يكون العنوان باللغة العربية، وأن يكون دقيقاً ومحدداً.
                    """),
                    ("user", """فيما يلي نص مستخرج من محاضرة إسلامية. يرجى تحديد الموضوع الرئيسي لهذا النص بعنوان دقيق وموجز:
...
def get_llm_client():
    ...
    try:
        if groq_api_key:
            ...
            return ChatGroq(temperature=0.2, model_name="llama3-70b-8192", groq_api_key=groq_api_key)
        else:
