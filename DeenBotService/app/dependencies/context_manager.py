...
def get_context_by_id(
    context_id: str, max_length: int = 100000
    ...
    try:
        ...
        if transcription_path.exists():
            ...
            try:
                ...
                if transcription:
                    logger.info(
                        f"Transcription récupérée ({len(transcription)} caractères)"
                    )
                    # Tronquer si nécessaire pour éviter des problèmes de mémoire
                    if len(transcription) > max_length:
                        logger.warning(
                            f"Transcription tronquée de {len(transcription)} à {max_length} caractères"
                        )
                        transcription = transcription[:max_length]
                    return transcription
            except Exception as e:
                logger.error(
                    f"Erreur lors de la lecture du fichier de transcription: {e}"
                )
        ...
        logger.info(
            f"Chargement de la collection ChromaDB avec filtre sur context_id: {context_id}"
        )
        try:
            ...
            for i, (doc, meta) in enumerate(zip(documents, metadatas)):
                if doc:  # Ignorer les documents vides
                    chunk_data = {
                        ...
                        "metadata": meta,
                        "embedding": (
                            embeddings_data[i] if i < len(embeddings_data) else None
                        ),
                    }
            ...
            if chunks_data:
                ...
                if len(full_context) > max_length:
                    logger.warning(
                        f"Contexte tronqué de {len(full_context)} à {max_length} caractères"
                    )
...
def semantic_chunking(
    text: str, min_chunk_size: int = 100, max_chunk_size: int = 500
) -> List[str]:
    """
            ...
            for sentence in sentences:
                if len(current_chunk) + len(sentence) <= max_chunk_size:
                    ...
                    if len(sentence) > max_chunk_size:
                        ...
                        chunks.extend(
                            sentence_chunks[:-1]
                        )  # Ajouter tous sauf le dernier
                        current_chunk = sentence_chunks[
                            -1
                        ]  # Commencer un nouveau chunk avec le dernier
                    else:
...
def calculate_keyword_score(
    text: str, keywords: List[str], lang_code: str = "fr"
) -> float:
    """
    ...
    if words_count > 0:
        normalization_factor = (
            np.sqrt(words_count) / 10
        )  # Division par 10 pour garder des scores raisonnables
...
def rank_chunks_by_relevance(
    question: str, chunks: List[str], lang_code: str = "fr"
...
def answer_from_context_only(
    ...
    try:
        logger.info(
            f"Génération de réponse avancée sans LLM pour la question: {question}"
        )
        ...
        if not chunks:
            ...
            if not chunks:
                chunks = [
                    context
                ]  # Utiliser tout le contexte comme un seul chunk si nécessaire
        ...
        if len(response) > max_response_length:
            ...
            if (
                last_period > max_response_length // 3
            ):  # Au moins un tiers de la taille maximale
        ...
        logger.info(
            f"Réponse générée sans LLM (version avancée): {len(response)} caractères"
        )
        ...
        logger.info(
            f"Retour de la réponse finale: {response[:50]}... (avec sources: {return_sources})"
        )
        if return_sources:
...
def rewrite_answer_with_llm(
    question: str, context_chunks: List[str], lang_code: str = "ar"
) -> str:
    """
...
- عند الاستشهاد بآية قرآنية موجودة في السياق:
  - انقلها حرفيًا كما وردت في السياق.
  - استخدم التنسيق التالي: **قال تعالى: «...الآية...» (اسم السورة: رقم الآية)**.
- عند ذكر حديث نبوي موجود في السياق:
  - اذكره بنصه الصحيح كما ورد في السياق.
...
"""
                ...
                lang_map = {"ar": "arabe", "en": "anglais", "fr": "français"}
...
- **Votre réponse doit être exclusivement en langue {target_language_name}.**"""
            ...
            if response.status_code == 200:
                ...
                if answer:
                    logger.info(
                        f"Réponse reformulée avec LLM: {len(answer)} caractères"
                    )
                    return answer
                else:
                ...
                logger.error(
                    f"Erreur API LLM: {response.status_code} - {response.text}"
                )
...
def get_answer_from_context(
    ...
    try:
        ...
        if use_llm:
            ...
            if llm_answer:
                if return_sources:
                    logger.info(f"llm_answeravec LLM return_sources=True: {llm_answer}")
                    return llm_answer, relevant_chunks
                logger.info(f"llm_answeravec LLM return_sources=False: {llm_answer}")
                return llm_answer
        ...
        if return_sources:
            answer, sources = answer_from_context_only(
                question, "\n\n".join(relevant_chunks), lang_code, return_sources=True
            )
            logger.info(
                f"answer_from_context_only answer return_sources=True: {llm_answer}"
            )
            return answer, relevant_chunks
        else:
            answer = answer_from_context_only(
                question, "\n\n".join(relevant_chunks), lang_code
            )
            logger.info(f"answer without LLM: {answer}")
            return answer
        ...
        logger.error(traceback.format_exc())
        error_msg = (
            "Désolé, une erreur s'est produite lors de la recherche de la réponse."
        )
